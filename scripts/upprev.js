...
+parseInt(box.css('padding-bottom').replace(/px$/,'')))+'px';}
function iub(box,iuh)
{return'-'+(box.width()
+iuh
+parseInt(box.css('padding-left').replace(/px$/,''))
+parseInt(box.css('padding-right').replace(/px$/,'')))+'px';}
jQuery(function($){if('undefined'==typeof(iworks_upprev)){return;}
var iuc=false;var iud=true;var iue=true;var iug=typeof(_gaq)!='undefined';var iuf=iworks_upprev.ga_opt_noninteraction==1;var iuh=0;function iui(){var ium=false;if(iworks_upprev.offset_element&&$(iworks_upprev.offset_element)){if($(iworks_upprev.offset_element).length>0){ium=iul()+$(window).height()>$(iworks_upprev.offset_element).offset().top;}else{ium=iul()+$(window).height()>=$(document).height()*iworks_upprev.offset_percent/100;}}else{ium=(iul()+$(window).height()>=$(document).height()*iworks_upprev.offset_percent/100);}
box=$('#upprev_box');if(ium&&!iuc){if(iworks_upprev.animation=="fade"){box.fadeIn("slow");}else{iuj=iworks_upprev.css_side+'px';switch(iworks_upprev.position.all){case'left':box.stop().animate({left:iuj,bottom:iworks_upprev.css_bottom+'px'});break;case'left-top':box.stop().animate({left:iuj,top:iworks_upprev.css_bottom+'px'});break;case'right':box.stop().animate({right:iuj,bottom:iworks_upprev.css_bottom+'px'});break;case'right-middle':box.css('top',(($(window).height()+box.height())/2)+'px');box.stop().animate({right:iuj});break;case'right-top':box.stop().animate({right:iuj,top:iworks_upprev.css_bottom+'px'});break;default:alert(iworks_upprev.position);break;}}
